...
#Form to take in an email on the landing page
class nameForm(FlaskForm):
    name = StringField('Email')
    submit = SubmitField('SUBMIT')
###Depracted will probably remove gives ability to attempt the dwolla re verification process
...
### Depracted will probably remove gives ability to decode
...
def templateEmailnew():
    ...
    query = sf.query("select Name, ID, (select email from contacts) from account where ID IN (select Account__c from project__c where project__c.Id  = '"+proj_ID+"')")
    email = quer['records'][0]['Contacts']['records'][0]['Email']
    ...
    address = str(address_query['records'][0]['MailingAddress']['street']) + " " + str(address_query['records'][0]['MailingAddress']['city']) + ", " + str(address_query['records'][0]['MailingAddress']['state'])
    ...
    msg['From'] = "nathan@certasun.com"#mail_settings['MAIL_USERNAME']
    msg['To'] = email
    ...
    with open('outgoing.msg', 'wb') as f:
        f.write(bytes(msg))
        smtp = smtplib.SMTP(AWS_SES_ENDPOINT, 25)
    smtp.ehlo()
    smtp.starttls()
    smtp.login(SMTP_Username, SMTP_Password)
    smtp.sendmail("nwayne1@luc.edu", email, msg.as_string())
    smtp.quit()
    return '', 200
...
def checkEmail(email):
    ...
    if query['totalSize'] == 0 or query['records'][0]['Projects__r'] == None:
        ...
        session['email'] = email
        return query['records'][0]['Projects__r']['records'][0]['Id']
...
def passObjID(id):
    ...
    session['proj_ID'] = id
    #email = session['email']
    query = sf.query("select account__r.id from project__c where id = '"+id+"'")
    if query['totalSize'] == 0:
        return(redirect(url_for('InvalidName')))
    accountID = query['records'][0]['Account__r']['Id']
    query = sf.query("select email from contact where account.id = '"+accountID+"'")
    email = query['records'][0]['Email']
    session['email'] = email
    ...
    if len(A_NAME_LIST) < 2:
        A_NAME_LIST.append("N/A")
    ...
    if passAccount:
        ...
        if passAccount.duePayment == 'Down Payment':
            ...
            charge_fee = 1
        return render_template('ach-credit.html',charge_fee = charge_fee, unverified_limit = DWOLLA_UNVERIFIED_LIMIT, accountobj = passAccount, payment = send_payment, email = email, location = location, pre_verified = pre_verified)
...
def runQueriesID(name, fname, lname, id):
    ...
    if thisAccount.dpSTAT == DUE_STATUS or thisAccount.dpSTAT == NOT_DUE_STATUS or thisAccount.dpSTAT == ' ':
        thisAccount.duePayment = DOWN_PAYMENT_CONST
        return thisAccount
    elif thisAccount.ppSTAT == DUE_STATUS or thisAccount.ppSTAT == NOT_DUE_STATUS or thisAccount.ppSTAT == ' ':
        thisAccount.duePayment = PERMIT_PAYMENT_CONST
        return thisAccount
    elif thisAccount.fpSTAT == DUE_STATUS or thisAccount.fpSTAT == NOT_DUE_STATUS or thisAccount.fpSTAT == ' ':
...
def create_verified_dwolla_customer():
    ...
    ##Charge under Dwolla veriffied limit so we will try to run it through dwolla
        ...
        if customer_response.body['total'] > 0:
            ...
            elif session['passAccount']['chargeAmount'] > DWOLLA_UNVERIFIED_LIMIT and customer_response.body['_embedded']['customers'][0]['status'] == 'unverified':
